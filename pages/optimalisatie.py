...
for solution in populatie:
    ...
    st.markdown("#### Objective Values:")
    st.markdown(f"- aanschafkosten: {st.session_state.aanschafprijs * f1(solution)} ")
    st.markdown(f"- onderhoudskosten: {st.session_state.onderhoudsprijs * f2(solution)}")
    st.markdown(f"- mate van losmaakbaarheid: {st.session_state.losmaakbaarheid * f3(solution)}")
    st.markdown(f"- toepassingsmogelijkheden: {st.session_state.toepassingsmogelijkheden * f4(solution)}")
    st.markdown(f"- woonbeleving: {st.session_state.woonbeleving * f5(solution)}")
    st.markdown(f"- milieubelasting: {st.session_state.milieubelasting * f6(solution)}")
    st.markdown(f"- flexibiliteit tbv toekomstbestendigheid en innovatie: {st.session_state.flexibiliteit * f7(solution)}")
    st.markdown(f"- mate van standaardisering: {st.session_state.standaardisering * f8(solution)}")
