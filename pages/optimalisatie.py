...
def composite_objective(x):
    # Weights for each individual objective function (positief voor maximaliseren, negatief voor minimaliseren)
    weights = [-1, -1, 1, 1, 1, -1, 1, 1]
    
    # Individual objective function values
    objectives = [f1(x), f2(x), f3(x), f4(x), f5(x), f6(x), f7(x), f8(x)]
    
    # Calculate the composite objective function value
    composite_value = sum(weight * objective for weight, objective in zip(weights, objectives))
...
objective_functions = [composite_objective]
